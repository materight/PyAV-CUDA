...
__global__ void cuda_nv12_to_rgb(uint8_t *in_y, uint8_t *in_uv, uint8_t *out_rgb, int height, int width) {
...
extern "C" {
    void nv12_to_rgb(uint8_t *in_y, uint8_t *in_uv, uint8_t *out_rgb, int height, int width) {
        ...
        cuda_nv12_to_rgb<<<grid, block>>>(in_y, in_uv, out_rgb, height, width);
